...
class UserController extends Controller
{
    ...
    public function getPeopleCode($ma){
        $request = new ApiRequestService();
        $response = $request->loginWithUserPass('tichhop', 'tichhop');
        // get people code
        if(isset($response['Success']) && $response['Success'] && isset($response['Data']['TokenCode'])){
            // token web server
            $token_server = $response['Data']['TokenCode'];
            $userInfo = $request->getInfoUser($ma, $token_server);
            if(isset($response['Success']) && $response['Success'] && isset($userInfo['Data']['PeopleCode'])){
                $people_code = $userInfo['Data']['PeopleCode'];
                return $people_code;
            }
        }
        return false;
...
//                'type' => 'required'
            ...
            [
                'sothe.required' => 'Số thẻ không được để trống',
                'password.required' => 'Bắt buộc phải nhập mật khẩu',
//                'type.required' => 'Vui lòng chọn loại mã bạn cần nhập',
        ...
        $peopleCode = $this->getPeopleCode($maThe);
        $request = new ApiRequestService();
        $response = $request->loginWithUserPass($peopleCode, $password);
        if(isset($response['Success']) && $response['Success'] && isset($response['Data']['TokenCode'])){
            $tokenUser = $response['Data']['TokenCode'];
            $response = $request->getInfoUser($maThe, $tokenUser);
            if(isset($response['Success']) && $response['Success'] && isset($response['Data']['PeopleCode'])){
    ...
    public function profile()
    {
        ...
        return view(config("edushop.end-user.pathView") . "profile")->with('user', $user);
    }
